                ...
                <div class="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-200 border-0">
                    <div class="rounded-t mb-0 px-6 py-6">
                        <div class="text-center mb-3">
                            <h6 class="text-y2 text-sm font-bold">
                                {{ __('global.register') }}
                            </h6>
                        </div>
                    ...
                    <div class="flex-auto px-4 lg:px-10 py-10 pt-0">
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div class="flex flex-row">
                                ...
                                <div class="relative w-full mb-3 mr-5">
                                    <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2" for="refcode">
                                       refcode{{-- {{ __('global.login_email') }}--}}
                                    </label>
                                    <input id="refcode" value="{{$refcode}}" name="refcode" type="text" class="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full {{ $errors->has('email') ? ' ring ring-red-300' : '' }}" placeholder="{{ __('global.login_email') }}"  />
                                    @error('email')
                                    <div class="text-red-500">
                                        <small>{{ $message }}</small>
                                    </div>
